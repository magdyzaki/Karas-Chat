...
async function getPool() {
  ...
  try {
    ...
    console.error('invite-pg init error:', e.message);
    return null;
  }
}

/** للتحقق من اتصال Postgres — يُستخدم في /api/invite-diagnostics */
export async function pgDiagnostics() {
  const hasUrl = !!process.env.DATABASE_URL;
  if (!hasUrl) return { hasDatabaseUrl: false, pgOk: false, error: 'DATABASE_URL غير مضبوط' };
  try {
    const p = await getPool();
    if (!p) return { hasDatabaseUrl: true, pgOk: false, error: 'فشل إنشاء الاتصال' };
    const count = await p.query('SELECT COUNT(*)::int AS n FROM invite_links');
    return { hasDatabaseUrl: true, pgOk: true, inviteCount: count.rows[0]?.n ?? 0 };
  } catch (e) {
    return { hasDatabaseUrl: true, pgOk: false, error: e.message };
  }
}
