...
httpServer.listen(PORT, HOST, async () => {
  console.log('Chat backend on', HOST + ':' + PORT);
  if (process.env.DATABASE_URL) {
    try {
      const pg = await import('./invite-pg.js');
      const d = await pg.pgDiagnostics?.();
      console.log('Postgres invite:', d?.pgOk ? 'OK, ' + (d.inviteCount ?? 0) + ' links' : (d?.error || 'unavailable'));
    } catch (e) {
      console.warn('Postgres init check:', e?.message);
    }
  }
});
