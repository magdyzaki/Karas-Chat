...
router.post('/register', async (req, res) => {
  ...
  if (INVITE_ONLY && !(phone && isTrustedPhone(phone))) {
    ...
    if (!link) {
      console.warn('[invite] Token not found:', token?.slice?.(0, 20) + '...');
      return res.status(403).json({ error: 'رمز الدعوة غير صالح. تأكد أن الرابط جديد وأنك أنشأته بعد ضبط DATABASE_URL.' });
    }
