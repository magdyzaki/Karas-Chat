...
/** توحيد الأرقام للمطابقة — 01X و 20X (مصر) يعتبران نفس الرقم */
function toCanonicalPhone(input) {
  const digits = String(input || '').replace(/\D/g, '');
  if (digits.length < 10) return '';
  if (digits.startsWith('01') && digits.length === 11) return '2' + digits;
  if (digits.startsWith('20') && digits.length >= 11) return digits.slice(0, 12);
  return digits;
}
...
export const db = {
  ...
  findUsersByPhones(phoneNumbers, excludeUserId = null) {
    ...
    for (const raw of phoneNumbers || []) {
      ...
      if (p) canonicalSet.add(p);
    }
    if (!canonicalSet.size) return [];
    const exclude = excludeUserId != null ? Number(excludeUserId) : null;
    return low.data.users
      .filter((u) => u.phone && canonicalSet.has(toCanonicalPhone(u.phone)) && (!exclude || u.id !== exclude))
      .map((u) => ({ id: u.id, email: u.email, phone: u.phone, name: u.name, avatar_url: u.avatar_url || null }));
  },
