...
# نحاول نجيب اسم الشركة من جدول العملاء
            company = company  # القيمة الافتراضية لو مالقيناش حاجة
            ...
            try:
                ...
                print("⚠️ لم يتم العثور على اسم الشركة:", e)
    ...
    def generate_invoice_word(self, preview=False):
        try:
            ...
            doc.add_paragraph(f"TO: {self.customer_data.get('company') or self.customer_data.get('name', '')}")

            # عنوان العميل - سطرين يدويين بحيث السطر الثاني يبدأ بعد كلمة ADDRESS:
            p_addr = doc.add_paragraph()
            r1 = p_addr.add_run("ADDRESS: ul. Rzeczna 2\n")
            r2 = p_addr.add_run("                     62-404 Ciążeń, Poland")  # ← هنا فيه 10 مسافات ثابتة

            p_addr.alignment = WD_ALIGN_PARAGRAPH.LEFT

            # رقم الهاتف
            p_tel = doc.add_paragraph(f"TEL NO: {self.customer_data.get('phone', '')}")
            p_tel.alignment = WD_ALIGN_PARAGRAPH.LEFT

            # تنسيق الخط
            for p in [p_addr, p_tel]:
                for r in p.runs:
                    r.font.size = Pt(16)
                    r.bold = True
            ...
            col_widths = [Inches(3.0), Inches(0.4), Inches(0.5), Inches(0.5)]
